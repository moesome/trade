...
public class RedisCacheManager implements CacheManager{
    ...
    public void saveCommodityDetailVo(CommodityDetailVo commodityDetailVo,Long expireSecond){
        ...
        String key = "commodityDetailVo"+commodityDetailVo.getId();
        redisTemplateForCommodityDetailVo.opsForHash().putAll(key,stringObjectHashMap);
        if (expireSecond > 0){
            redisTemplateForCommodityDetailVo.expire(key,expireSecond,TimeUnit.SECONDS);
        }
    }
     ...
     */
    @Override
    public CommodityDetailVo getCommodityDetailVo(Long spikeId){
        // 从缓存取信息用于校验
        ...
        list.add("createdAt");
        list.add("updatedAt");
        List<Object> multiGet = redisTemplateForCommodityDetailVo.opsForHash().multiGet("commodityDetailVo" + spikeId,list);
        ...
        commodityDetailVo.setId(spikeId);
